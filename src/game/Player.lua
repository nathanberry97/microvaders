...
function Player:shootAnimation(laserShooting)
    if pd.buttonJustPressed(pd.kButtonA) and not self.shooting and not laserShooting then
        self.shooting = true
        self.targetTopY = self.baseY + 3
    elseif pd.buttonJustReleased(pd.kButtonA) and self.shooting then
        self.shooting = false
        self.targetTopY = self.baseY
    end

    if self.topY < self.targetTopY then
        self.topY += 1
    elseif self.topY > self.targetTopY then
        self.topY -= 1
    end
end
